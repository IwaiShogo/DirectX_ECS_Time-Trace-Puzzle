...
private:
	...
	static void SerializeComponent(Registry& reg, Entity e, json& j, const std::string& key) {
		if (reg.has<T>(e)) {
			T& comp = reg.get<T>(e);
			j[key] = ToJson(comp); // 各コンポーネントごとの変換関数を呼ぶ
		}
	}
	...
	static void FromJson(const json& j, Tag& c) { c.name = j["name"].get<std::string>().c_str(); }
	...
	static json ToJson(const MeshComponent& c) {
		return { {"key", c.modelKey}, {"color", {c.color.x, c.color.y, c.color.z, c.color.w}} };
	}
	...
	static json ToJson(const AudioSource& c) {
		return { {"key", c.soundKey}, {"vol", c.volume}, {"range", c.range}, {"loop", c.isLoop} };
	}
